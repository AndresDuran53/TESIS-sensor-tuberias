...
def getTablaAnterior(nick):
    lista=[]
    db = pymysql.connect("127.0.0.1", "root", "1234", "BD_Alcantarilla")
    curs=db.cursor()
    curs.execute ("SELECT altura_leida, hora_registro, FROM tbl_material WHERE nickname_tuberia = 'tub1'")
    for reading in curs.fetchall():
        print("Altura: ",reading[0])
        print("Hora: ",reading[1])
